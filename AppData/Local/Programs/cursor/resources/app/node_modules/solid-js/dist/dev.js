...
function readSignal() {
  ...
  if (Listener) {
    ...
    if (!this.observers) {
      ...
      if (DEV && this.observers.length > 2000 && this.observers.length % 500 === 0) {
        const trace = new Error().stack?.split('\n').slice(1).join('\n');
        console.warn(`solid signal has`, this.observers.length, "observers for valueType:", typeof this.value, trace);
        try {
          let valueStr;
          try {
            valueStr = typeof this.value === 'object' && this.value !== null
              ? Object.prototype.toString.call(this.value)
              : String(this.value);
          } catch (e) {
            valueStr = '[unserializable value]';
          }
          postMessage({
            type: 'solid-js-observer-count',
            value: valueStr,
            observers: this.observers.length,
            trace
          });
        } catch (e) {
          // Ignore postMessage errors for non-cloneable values
        }
      }
    }
  }
...
export { $DEVCOMP, $PROXY, $TRACK, DEV, ErrorBoundary, For, Index, Match, Show, Suspense, SuspenseList, Switch, batch, cancelCallback, catchError, children, cleanNode, createComponent, createComputed, createContext, createDeferred, createEffect, createMemo, createReaction, createRenderEffect, createResource, createRoot, createSelector, createSignal, createUniqueId, enableExternalSource, enableHydration, enableScheduling, equalFn, from, getListener, getOwner, indexArray, lazy, mapArray, mergeProps, observable, on, onCleanup, onError, onMount, requestCallback, resetErrorBoundaries, runWithOwner, sharedConfig, splitProps, startTransition, untrack, useContext, useTransition };
